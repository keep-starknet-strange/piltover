...
pub fn decode_output(mut os_output: Vec<Felt252>) -> Result<StarknetOsOutput, SnOsError> {
    ...
    // Extract the messages count from the first element of os_output
    let messages_count = felt_vm2usize(os_output.first()).unwrap_or(0);
    // Remove the messages count from os_output
    os_output.remove(0);

    // Extract messages_to_l1 using the messages count
    let messages_to_l1: Vec<Felt252> = os_output.drain(..messages_count).collect();
    // Extract messages_to_l2 using the messages count
    let messages_to_l2: Vec<Felt252> = os_output.drain(..messages_count).collect();
    ...
    Ok(StarknetOsOutput {
        ...
        messages_to_l1,
        messages_to_l2,
        state_updates: os_output.drain(..felt_vm2usize(os_output.first())?).collect(),
        contract_class_diff: os_output,
    })
}
